...
vector<pair<long long,long long> > F[hashkey+10];
...
long long rec(long long i,int j)
{
    ...
    long long hcode = code % hashkey;
    FOREACH(it,F[hcode]) if ((*it).first == code) return (*it).second;
    ...
    F[hcode].push_back(make_pair(code,sum));
    return sum;
}
int main()
{

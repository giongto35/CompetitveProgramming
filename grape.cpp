...
int calc(int left,int right)
{
    ...
    {
        mid=(l+r)/2;
        if (Val[mid][right]>=left) kq=mid,l=mid+1;
        else r=mid-1;
    }
    return kq;
}
...
int main()
{
    //freopen("grape_12.inp","r",stdin);
    //freopen("grape_12.out","w",stdout);
    scanf("%d%d%d",&m,&n,&q);
    FOR (i,1,m)
    FOR (j,1,n)
    {
    ...
    FOR(k,1,min(m,n))
    {
        ...
        FOR(i,k,m)
        FOR(j,k,n){
            GTmin[i][j][t]=min(min(GTmin[i][j-1][1-t],GTmin[i-1][j][1-t]),min(a[i][j],GTmin[i-1][j-1][1-t]));
            GTmax[i][j][t]=max(max(GTmax[i][j-1][1-t],GTmax[i-1][j][1-t]),max(a[i][j],GTmax[i-1][j-1][1-t]));
            Val[k][GTmax[i][j][t]]=max(Val[k][GTmax[i][j][t]],GTmin[i][j][t]);
        }
        FOR(i,1,maxso)

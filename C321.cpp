...
void DFS(int u)
{
    ...
    {
        int v=a[u][i];
        if (ok[(long long)u*(n+1)+v]&&used[v]!=cur)
            DFS(v);
    }
}
...
int FindCenter()
{
    ...
    while (fr<re)
    {
        ...
        {
            int v=a[u][j];
            if (ok[(long long)u*(n+1)+v]&&dx[v]!=cur)
            {
                deg[v]--;
...
void compute(int r,char kt)
{
    ...
    FOR(i,0,a[u].size()-1)
    {
        ...
        if (ok[(long long)u*(n+1)+v])
        {
            //cout<<"xet tiep "<<v<<endl;
            ok[(long long)u*(n+1)+v]=false;
            ok[(long long)v*(n+1)+u]=false;
            compute(v,kt+1);
        }
    }
}
...
int main()
{
    ...
    FOR(i,1,n-1)
    {
        ...
        a[u].push_back(v);
        a[v].push_back(u);
        ok[(long long)u*(n+1)+v]=true;
        ok[(long long)v*(n+1)+u]=true;
    }
    compute(1,'A');
    FOR(i,1,n) printf("%c ",res[i]);//cout<<res[i]<<' ';
    fclose(stdin);
    fclose(stdout);
    return 0;
}

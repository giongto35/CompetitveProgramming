...
inline int gethash(long long n)
{
    ...
    return hash[(n+13)%cmod];
    //return 0;
}

inline void updatehash(long long n,int gt)
{
    //int c=n%cmod;
    //FOR(i,0,hash[c].size()-1)
...
inline void bit_add(int i,int jj,int gt)
{
    int res=0;
    while (i<=n)
    {
        ...
        while (j<=n)
        {
            ...
            updatehash((long long)i*(n+1)+j,gt);
            j=j+(j&(-j));
        }
        i=i+(i&(-i));
    }

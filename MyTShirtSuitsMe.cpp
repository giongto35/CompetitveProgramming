...
edge make_edge(const int x,const int y, const int c)
{
    edge res;
    res.x = x;
    res.y = y;
...
int main()
{
    //freopen("MyTShirtSuitsMe.inp","r",stdin);
    ...
    while (TC>0){
        ...
        {
            cin>>u1>>u2;
            e[++m] = make_edge(i,M+CH[u1],1);
            e[++m] = make_edge(i,M+CH[u2],1);
        }
        FOR(i,1,M)
            e[++m] = make_edge(s,i,1);
        FOR(i,1,6)
            e[++m] = make_edge(M+i,t,N / 6);
        //cout<<m<<' '<<N/6<<endl;
        if (RunFlow() == M) cout<<"YES"<<endl; else cout<<"NO"<<endl;
        TC--;
    }
    fclose(stdin);
    fclose(stdout);
